...
#### Using a Hugging Face Dataset (no JSONL)

If you don't want to use JSONL, you can provide a Hugging Face `datasets.Dataset` instead.

- **From the Hub (or a local dataset script)**: set `training_huggingface_dataset` / `validation_huggingface_dataset`.
- **From disk**: if you've saved a dataset via `dataset.save_to_disk(...)`, set `training_huggingface_disk_path` / `validation_huggingface_disk_path`.

Once a validation dataset is provided, you can enable evaluation by setting `evaluation_strategy` in `trainer_config.hf_trainer_config` (note: older configs may use `eval_strategy`, which is still accepted).
