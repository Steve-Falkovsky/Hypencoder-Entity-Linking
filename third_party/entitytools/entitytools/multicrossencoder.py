...
def create_multidataset(collection, candidates_lookup, ontology, tokenizer, max_length, top_k=5, no_labels=False):
    """
        Creates the dataset for the multi cross-encoder to process.
            ...
            for sentence in passage.sentences:
                ...
                for anno in sentence_annotations:
                    ...
                    for onto_rank, onto_idx in enumerate(candidates):
                        ...
                        aux_text = f"{multi_text}{tokenizer.sep_token}{anno.text}{tokenizer.mask_token}{entity['name']}"
                        single_text = f"{sentence_text}{tokenizer.sep_token}{anno.text}{tokenizer.mask_token}{entity['name']}"

                        tk = tokenizer.tokenize(aux_text)
                        if len(tk) >= max_length and aux_text == single_text:
                            continue
                        elif len(tk) >= max_length:
                            multi_dataset.append((multi_labels, multi_text, multi_annos))
                            multi_labels, multi_annos = [], []
                            multi_text = f"{sentence_text}{tokenizer.sep_token}{anno.text}{tokenizer.mask_token}{entity['name']}"
                        elif len(tk) < max_length:
                            multi_text = aux_text
...
def apply_multicrossencoder(
    ...
    """
    Applies the multi cross-encoder to generate mention-candidate scores.
    ...
    :return: the time needed to apply the multi cross-encoder
    """
    ...
    data_multi = create_multidataset(collection, candidates_lookup, ontology, tokenizer, crossencoder_size, top_k=top_k,
                                     no_labels=True)
    batch_size = 2
...
def train_multicrossencoder(
    ...
    """
        Trains the multi cross-encoder to generate mention-candidate scores.
    ...
    print("Creating datasets ready for base cross-encoder")
    train_data = create_multidataset(train_collection, candidates_lookup, ontology, tokenizer, size, top_k=top_k)
    val_data = create_multidataset(val_collection, candidates_lookup, ontology, tokenizer, size, top_k=top_k)
