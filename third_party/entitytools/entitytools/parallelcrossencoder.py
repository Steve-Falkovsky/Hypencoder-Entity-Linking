...
def create_paralleldataset(collection, candidates_lookup, ontology, tokenizer, max_length, top_k=5, no_labels=False):
    """
...
def apply_parallelcrossencoder(
        collection,
        ontology,
        candidates_lookup,
        top_k,
    ...
    """
        Applies the parallel cross-encoder to generate mention-candidate scores.
        ...
        :return: the time needed to apply the parallel cross-encoder
    """
    ...
    data_multi = create_paralleldataset(collection, candidates_lookup, ontology, tokenizer, crossencoder_size,
...
def train_parallelcrossencoder(
        train_collection,
        val_collection,
        ontology,
        candidates_lookup,
    ...
    """
        Trains the parallel cross-encoder to generate mention-candidate scores.
    ...
    train_data = create_paralleldataset(train_collection, candidates_lookup, ontology, tokenizer, size, top_k=top_k)
    val_data = create_paralleldataset(val_collection, candidates_lookup, ontology, tokenizer, size, top_k=top_k)
