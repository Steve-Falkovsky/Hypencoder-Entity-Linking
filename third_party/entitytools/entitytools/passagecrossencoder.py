...
def create_passagedataset(collection, candidates_lookup, ontology, tokenizer, max_length, top_k=5, no_labels=False):
    """
    ...
    for doc in tqdm(collection.documents):
        for passage in doc.passages:
            ...
            for sentence in passage.sentences:
                ...
                if multi_text == "":
                    aux_text = sentence_text
                else:
                    aux_text = f"{multi_text}{tokenizer.sep_token}{sentence_text}"

                tk = tokenizer.tokenize(aux_text)
                if len(tk) >= max_length and aux_text == sentence_text:
                    continue
                elif len(tk) >= max_length:
                    multi_dataset.append((multi_labels, multi_text, multi_annos))
                    multi_labels, multi_annos = [], []
                    aux_text = sentence_text
                    multi_text = ""
                ...
                for anno in sentence_annotations:
                    ...
                    for onto_rank, onto_idx in enumerate(candidates):
                        ...
                        aux_text_2 = f"{aux_text}{tokenizer.sep_token}{anno.text}{tokenizer.mask_token}{entity['name']}"
                        single_text = f"{sentence_text}{tokenizer.sep_token}{anno.text}{tokenizer.mask_token}{entity['name']}"

                        tk = tokenizer.tokenize(aux_text_2)
                        if len(tk) >= max_length and aux_text_2 == single_text:
                            continue
                        elif len(tk) >= max_length:
                            multi_dataset.append((multi_labels, multi_text, multi_annos))
                            multi_labels, multi_annos = [], []
                            multi_text = single_text
                            aux_text = single_text
                        elif len(tk) < max_length:
                            multi_text = aux_text_2
                            aux_text = aux_text_2
                        multi_labels.append(label)
                        multi_annos.append((anno, onto_rank, onto_idx))
...
def apply_passagecrossencoder(
    ...
    """
    Applies the passage cross-encoder to generate mention-candidate scores.
    ...
    data_multi = create_passagedataset(collection, candidates_lookup, ontology, tokenizer, crossencoder_size,
...
def train_passagecrossencoder(
    ...
    """
    Trains the passage cross-encoder to generate mention-candidate scores.
    ...
    :return: the time needed to train the passage cross-encoder
    """
    gc.collect()
    ...
    train_data = create_passagedataset(train_collection, candidates_lookup, ontology, tokenizer, size, top_k=top_k)
    val_data = create_passagedataset(val_collection, candidates_lookup, ontology, tokenizer, size, top_k=top_k)
