...
def create_documentdataset(collection, candidates_lookup, ontology, tokenizer, max_length, top_k=5, no_labels=False):
    """
...
def apply_documentcrossencoder(
        collection,
        ontology,
        candidates_lookup,
        top_k,
    ...
    """
    Applies the document cross-encoder to generate mention-candidate scores.
    ...
    data_multi = create_documentdataset(collection, candidates_lookup, ontology, tokenizer, crossencoder_size,
...
def train_documentcrossencoder(
        train_collection,
        val_collection,
        ontology,
        candidates_lookup,
    ...
    """
        Trains the document cross-encoder to generate mention-candidate scores.
    ...
    train_data = create_documentdataset(train_collection, candidates_lookup, ontology, tokenizer, size, top_k=top_k)
    val_data = create_documentdataset(val_collection, candidates_lookup, ontology, tokenizer, size, top_k=top_k)
